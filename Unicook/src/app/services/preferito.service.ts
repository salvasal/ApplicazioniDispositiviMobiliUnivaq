...
export class PreferitoService {
  ...
  update(preferito: Preferito): Observable<Preferito> {
    return this.http.put<Preferito>(URL_API.singlePreferito(preferito.IDpreferito), preferito);
  }
  ...
  delete(id: number): Observable<Preferito> {
    return this.http.delete<Preferito>(URL_API.singlePreferito(id));
  }
}
